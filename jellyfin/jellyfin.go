...
func (j *Jellyfin) RegisterHandlers(s *mux.Router) {
	...
	r.Handle("/System/ActivityLog/Entries", m(j.systemActivityLogEntriesHandler))
	r.Handle("/System/Configuration", m(j.systemConfigurationHandler))
	r.Handle("/System/Configuration/Network", m(j.systemConfigurationNetworkHandler))
	r.Handle("/Environment/Drives", m(j.environmentDrivesHandler))
	...
	r.Handle("/Artists", m(j.artistsHandler)).Methods("GET")
	r.Handle("/Artists/{name}", m(j.artistHandler)).Methods("GET")
