		...
		for _, studio := range item.Studios {
			if studio.Name != "" {
				if _, exists := studioSet[studio.ID]; !exists {
					...
					if studioItem, err := j.makeJFItemStudio(r.Context(), reqCtx.User.ID, studio.ID); err == nil {
...
func (j *Jellyfin) studioHandler(w http.ResponseWriter, r *http.Request) {
	...
	response, err := j.makeJFItemStudio(r.Context(), reqCtx.User.ID, makeJFStudioID(studio))
	if err != nil {
...
func (j *Jellyfin) makeJFItemStudio(ctx context.Context, userID string, studioID string) (JFItem, error) {
	...
	response := JFItem{
		...
		LockedFields:      []string{},
		UserData:          j.makeJFUserData(userID, studioID, nil),
	}
	return response, nil
}
